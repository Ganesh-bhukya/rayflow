...
export async function createRazorpayOrder(input) {
    ...
    if (!input.currency || input.currency.length !== 3) {
        throw new Error("Razorpay order currency must be a 3-letter ISO code.");
    }
    if (!input.receipt ||
        input.receipt.length > 40) {
        throw new Error("Razorpay order receipt is required and must be at most 40 characters.");
    }
    const orderPayload = {
        amount: input.amount,
...
export async function createRazorpayRefund(input) {
    ...
    if (!input.currency || input.currency.length !== 3) {
        throw new Error("Razorpay refund currency must be a 3-letter ISO code.");
    }
    if (!input.receipt ||
        input.receipt.length > 40) {
        throw new Error("Razorpay refund receipt is required and must be at most 40 characters.");
    }
    /*
    ...
    const basicAuth = Buffer.from(`${razorpayConfig.keyId}:${razorpayConfig.keySecret}`).toString("base64");
    const refundPayload = {
        amount: input.amount,
        speed: "normal",
        receipt: input.receipt,
     ...
     */
    const idempotencyKey = `rayflow-refund-${input.receipt}`;
    ...
    if (!response.ok) {
        const razorpayMessage = data?.error?.description ||
            data?.error?.reason ||
            data?.message ||
            `Razorpay refund failed with HTTP ${response.status}.`;
        const error = new Error(razorpayMessage);
    ...
    return {
        ...
        paymentId: data.payment_id ??
            input.paymentId,
        status: data.status,
        speedProcessed: data.speed_processed ??
            null,
        speedRequested: data.speed_requested ??
            null,
