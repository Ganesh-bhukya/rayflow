  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        status: "error",
        error:
          "eventId, eventType, orderId and attemptId are required",
      });
    }
 ...
 */
function getRazorpayRefundEntity(
  payload: any,
): any | null {
  return (
 ...
 */
function getRayFlowRefundId(
  refundEntity: any,
): string | null {
  const value =
    refundEntity?.notes?.rayflowRefundId;
...
router.post(
  ...
  if (!rawBody) {
    console.error(
      "Razorpay webhook rejected: raw body unavailable.",
    );
  ...
  if (
    !supportedRefundEvents.includes(eventType)
  ) {
    /*
  ...
  const refundEntity =
    getRazorpayRefundEntity(payload);

  if (!refundEntity) {
    return res.status(400).json({
      status: "error",
      error:
        "Razorpay refund entity not found",
    });
  }

  const rayFlowRefundId =
    getRayFlowRefundId(refundEntity);
  ...
  if (!rayFlowRefundId) {
    ...
    console.error(
      "Razorpay refund webhook missing RayFlow refund ID.",
      {
        eventId: razorpayEventId,
        eventType,
    ...
    return res.status(400).json({
      status: "error",
      error:
        "RayFlow refund ID missing from Razorpay refund notes",
    });
  }
  ...
  try {
    ...
    if (existingEvent.rows.length > 0) {
      ...
      return res.status(200).json({
        ...
        eventType,
        message:
          "Razorpay webhook already received",
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        status: "error",
        error:
          "Razorpay refund amount does not match RayFlow refund",
      });
    }
