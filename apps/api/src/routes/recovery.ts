...
import {
  ...
  type RecoveryAction,
} from "../services/recoveryDecisionEngine.js";
...
router.get("/", async (_req, res) => {
  ...
  try {
    ...
    for (
      const payment of paymentsResult.rows
    ) {
      const reason =
    ...
    try {
      ...
      if (
        ...
        return res.status(200).json({
          ...
          message:
            "Recovery retry was blocked by the automation safety policy.",
...
router.get("/intelligence", async (_req, res) => {
  ...
  try {
     ...
     */
    const completedRecoveriesResult =
      await pool.query(`
    ...
    const completedRecoveries =
      completedRecoveriesResult.rows
        .map((row) => {
          let metadata: Record<
            string,
            unknown
    ...
    const projectedRecoveryOpportunity =
      decisions
        .filter((decision) => decision.action === "RETRY")
        .reduce(
          (total, decision) =>
            total + decision.expectedRecoveryAmount,
          0,
        );

    const escalationAmount =
      actionSummary.ESCALATE.amount;

    const stoppedAmount =
      actionSummary.STOP.amount;
    ...
    const averageRecoveryProbability =
      decisions.length > 0
        ? Number(
            (decisions.reduce(
              (total, decision) =>
                total + decision.recoveryProbability,
              0,
            ) / decisions.length).toFixed(4),
          )
        : 0;
    ...
    console.error(
      "Failed to build recovery intelligence:",
      error,
    );
...
|--------------------------------------------------------------------------
| Read-only recovery simulation.
|
| Evaluates the same current failed-payment population and the same
| bounded decision engine used by the real recovery workflow, but:
|
  ...
  try {
    ...
    const decisions = failedPaymentsResult.rows.map(
      (payment) => {
        ...
        actionBreakdown[decision.action].cases += 1;
        actionBreakdown[decision.action].amount += amount;
    ...
    const projectedRecoveryOpportunity =
      failedPaymentsResult.rows.reduce(
        (total, payment) => {
          const previousFailedAttempts = Number(
            payment.previous_failed_attempts ?? 0,
          );
