...
import { createRazorpayRefund, } from "../infrastructure/razorpay/razorpayClient.js";
...
router.post("/", async (req, res) => {
    ...
    try {
         ...
         */
        if (String(transaction.status).toLowerCase() !==
            "success") {
            await client.query("ROLLBACK");
         ...
         */
        if (String(transaction.type).toLowerCase() !==
            "payment") {
            await client.query("ROLLBACK");
            return res.status(409).json({
                status: "error",
        ...
        const razorpayPaymentId = String(attempt.provider_reference ??
            "").trim();
        if (!razorpayPaymentId) {
            await client.query("ROLLBACK");
            return res.status(409).json({
                status: "error",
                error: "This transaction is not linked to a Razorpay payment",
                details: {
                    transactionId,
        ...
        `, [transactionId]);
        const alreadyRefunded = Number(refundedResult.rows[0]
            ?.refundedAmount || 0);
        const transactionAmount = Number(transaction.amount);
        const remainingRefundable = transactionAmount -
            alreadyRefunded;
        /*
         * --------------------------------------------------------
         * STEP 4
        ...
        try {
            razorpayRefund =
                await createRazorpayRefund({
                    ...
                    receipt: `rf_${refund.id.replace(/-/g, "").slice(0, 32)}`,
                    notes: {
                        rayflowRefundId: refund.id,
                        rayflowTransactionId: transactionId,
                        rayflowOrderId: transaction.order_id,
                    },
                });
        }
        catch (razorpayError) {
         ...
         * --------------------------------------------------------
         */
        const razorpayStatus = String(razorpayRefund.status ||
            "").toLowerCase();
        const rayflowRefundStatus = razorpayStatus ===
            "processed"
            ? "success"
            : razorpayStatus ===
                "failed"
         ...
         * --------------------------------------------------------
         */
        const totalRefundResult = await client.query(`
        SELECT
          COALESCE(
            SUM(amount) FILTER (
        ...
        WHERE transaction_id = $1
        `, [transactionId]);
        const totalRefunded = Number(totalRefundResult.rows[0]
            ?.totalRefunded || 0);
        const refundState = totalRefunded >=
            transactionAmount
            ? "refunded"
            : "partially_refunded";
        /*
      ...
      `, [
            ...
            JSON.stringify({
                ...
                razorpaySpeedRequested: razorpayRefund.speedRequested,
                razorpaySpeedProcessed: razorpayRefund.speedProcessed,
                refundState,
                reason: reason || null,
            }),
        ]);
        ...
        return res.status(201).json({
            ...
            refund: {
                ...
                alreadyRefunded,
                totalRefunded,
                remainingRefundable: Math.max(transactionAmount -
                    totalRefunded, 0),
                refundState,
                razorpayPaymentId,
                razorpayRefundId: razorpayRefund.id,
        ...
        try {
            ...
            console.error("Refund rollback error:", rollbackError);
        }
...
router.post("/:id/complete", async (req, res) => {
    ...
    try {
         ...
         */
        if (String(refund.status).toLowerCase() ===
            "success") {
            await client.query("COMMIT");
            return res.json({
                status: "ok",
        ...
        return res.status(409).json({
            status: "error",
            error: "Manual refund completion is disabled. Refunds are processed through Razorpay.",
        });
    }
    catch (error) {
        try {
