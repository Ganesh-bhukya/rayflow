...
router.post("/razorpay/verify", async (req, res) => {
  try {
    ...
    if (
      typeof rayflowPaymentId !== "string" ||
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const razorpayOrder =
      await createRayFlowRazorpayOrder({
        orderId,
    ...
    const paymentWithRazorpayOrder =
      razorpayUpdateResult.rows[0];
    ...
    return res.status(201).json({
      ...
      message:
        "Payment and Razorpay order created successfully",
