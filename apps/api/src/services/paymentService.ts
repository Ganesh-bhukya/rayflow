    ...
    if (order.status === "paid") {
      ...
      return {
        ...
        payment: {
          ...
          status: order.status,
          attemptId:
            existingAttempt.rows[0]?.id ?? null,
          paymentMethod:
            existingAttempt.rows[0]?.payment_method ??
            paymentMethod,
        },
      };
    }
...
const providerResult =
  await getPaymentStatus(providerReference);
    /*
     * ============================================================
     * PROVIDER SUCCESS
     * ============================================================
    ...
    if (providerResult.status === "success") {
      ...
      if (
        (updatedAttemptResult.rowCount ?? 0) === 0
      ) {
        throw new Error(
          "Payment attempt could not be completed",
        );
      }
      ...
      if (
        (updatedOrderResult.rowCount ?? 0) > 0
      ) {
      ...
      return {
        ...
        payment: {
          ...
          paymentMethod:
            successfulAttempt.payment_method,
          providerReference:
            successfulAttempt.provider_reference,
          transactionId,
        },
      };
    }
