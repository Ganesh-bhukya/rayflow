...
function safeEqualHex(expectedSignature, receivedSignature) {
...
export async function verifyRazorpayPayment(input) {
    ...
    try {
         ...
         * ------------------------------------------------------------
         */
        if (!input.rayflowPaymentId ||
            !input.razorpayPaymentId ||
            !input.razorpayOrderId ||
            !input.razorpaySignature) {
            throw new Error("All Razorpay verification fields are required.");
        }
        /*
         * ------------------------------------------------------------
         * STEP 2
      ...
      `, [input.rayflowPaymentId]);
        if (orderResult.rows.length === 0) {
            ...
            throw new Error("RayFlow payment order not found.");
        }
        ...
        if (!order.razorpay_order_id) {
            await client.query("ROLLBACK");
            throw new Error("Razorpay order ID is missing for this payment.");
        }
        if (order.razorpay_order_id !==
            input.razorpayOrderId) {
            await client.query("ROLLBACK");
            throw new Error("Razorpay order ID does not match the RayFlow payment.");
        }
        /*
         * ------------------------------------------------------------
         * STEP 4
        ...
        if (order.status === "paid") {
          ...
          `, [order.id]);
            ...
            return {
                ...
                attemptId: existingResult.rows[0]?.attemptId ?? "",
                transactionId: existingResult.rows[0]?.transactionId ??
                    null,
                status: order.status,
            };
        }
         ...
         * ------------------------------------------------------------
         */
        const generatedSignature = generatePaymentSignature(order.razorpay_order_id, input.razorpayPaymentId);
        const signatureValid = safeEqualHex(generatedSignature, input.razorpaySignature);
        if (!signatureValid) {
            /*
        ...
        if (attempt.status !== "success") {
            assertTransition(attempt.status, "success");
            await client.query(`
      ...
      `, [
        ...
        try {
            ...
            console.error("Razorpay verification rollback error:", rollbackError);
        }
        throw error;
    }
    finally {
...
//# sourceMappingURL=razorpayVerificationService.js.map