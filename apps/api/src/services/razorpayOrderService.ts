...
export async function createRayFlowRazorpayOrder(
  input: CreateRayFlowRazorpayOrderInput,
  ...
  if (!Number.isInteger(input.amount) || input.amount <= 0) {
    throw new Error(
      "RayFlow payment amount must be a positive integer.",
    );
  }

  if (!input.currency || input.currency.length !== 3) {
    throw new Error(
      "RayFlow payment currency must be a 3-letter ISO code.",
    );
  }
